...
async def handle_extra(update, context):
    ...
    await query.answer()        # ===== Drawer Open =====    if data_cb.startswith("drawer:open:"):        _, _, path, idx = data_cb.split(":")        idx = int(idx)        drawer_states[(path, idx, user_id)] = True        await query.message.edit_reply_markup(            get_extra_keyboard(path, idx, True, user_id)        )        return    # ===== Drawer Close =====    if data_cb.startswith("drawer:close:"):        _, _, path, idx = data_cb.split(":")        idx = int(idx)        drawer_states[(path, idx, user_id)] = False        await query.message.edit_reply_markup(            get_extra_keyboard(path, idx, True, user_id)        )        return

    # تجاهل ضغط الضيف
    if data_cb == "ignore":
        return
    # =========================    # Drawer toggle (المجر)    # =========================    if data_cb.startswith("drawer:"):        parts = data_cb.split(":")        path = parts[1]        idx = int(parts[2])        # خزن حالة الفتح لكل مستخدم        drawer_states.setdefault(user_id, set())        if idx in drawer_states[user_id]:            drawer_states[user_id].remove(idx)        else:            drawer_states[user_id].add(idx)        # تحديث الإعلان فقط        await refresh_everywhere(context, path, idx)        return
    # =========================
    # تغيير الحالة
    # =========================
...
def register(app):
    ...
    app.add_handler(        CallbackQueryHandler(            handle_extra,            pattern="^(status:|edit:|editcancel|ignore|quickdel:|drawer:)"        )    )
